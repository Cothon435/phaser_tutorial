...
import { ASSET_KEYS, CHEST_REWARD_TO_TEXTURE_FRAME, DEPTH } from '../common/assets';
import { Player } from '../game-objects/player/Player';
import { KeyboardComponent } from '../components/input/keyboard-component';
import { Spider } from '../game-objects/enemies/spider';
import { Wisp } from '../game-objects/enemies/wisp';
import { CharacterGameObject } from '../game-objects/common/character-game-object';
import { CHEST_REWARD_TO_DIALOG_MAP, DIRECTION, LEVEL_NAME, LEVEL_NAME_LIST } from '../common/common';
...
export class GameScene extends Phaser.Scene {
  ...
  public create(): void {
    if (!this.input.keyboard) {
      throw new DOMException('Phaser keyboard plugin is not setup properly.');
    }
  ...
  #registerColliders(): void {
    ...
    this.physics.add.collider(this.#player, this.#lockedDoorGroup, (player, gameObject) => {
      ...
      const doorId = parseInt(doorObject.name);
      const door = this.#objectsByRoomId[this.#currentRoomId]!.doorMap[doorId] as Door;
    ...
    Object.keys(this.#objectsByRoomId).forEach((key) => {
      ...
      if (this.#objectsByRoomId[roomId].enemyGroup !== undefined) {
        ...
        this.physics.add.overlap(this.#player, this.#objectsByRoomId[roomId].enemyGroup, (player, enemy) => {
          this.#player.hit(DIRECTION.DOWN, 3);
        });
  ...
  #createLevel(): void {
    // const backGroundKey = `${this.#levelData.level}_BACKGROUND` as keyof typeof ASSET_KEYS;
    // const forGroundKey = `${this.#levelData.level}_FOREGROUND` as keyof typeof ASSET_KEYS;
    ...
    // const tiles = this.#map.addTilesetImage(ASSET_KEYS[`${this.#levelData.level}_TILESET`]);
    const tiles = this.#map.addTilesetImage(
      ASSET_KEYS[`${this.#levelData.level}_TILESET_IMG`],
      ASSET_KEYS[`${this.#levelData.level}_TILESET_IMGKEY`]
    );
    const lenth = LEVEL_NAME_LIST[this.#levelData.level].length;
    this.#levelData.layerNameList.forEach((layerName, index) => {
      const layer = this.#map.createLayer(layerName, tiles!, 0, 0);
      layer?.setDepth(lenth - index);
      console.log('Lenth: {}', lenth - index);
    });

    // const backgound_layer = this.#map.createLayer('dungeon_background', tiles!, 0, 0);
    // backgound_layer!.setPosition(Math.round(backgound_layer!.x), Math.round(backgound_layer!.y));
    // backgound_layer?.setDepth(DEPTH.BACK_GROUND);
    // const forground_layer = this.#map.createLayer('foreground', tiles!, 0, 0);
    // forground_layer?.setDepth(DEPTH.FOR_GROUND);

    // The first parameter is the name of the tileset in Tiled and the second parameter is the key
    // of the tileset image used when loading the file in preload.
    const collisionTiles = this.#map.addTilesetImage(TILED_TILESET_NAMES.COLLISION, ASSET_KEYS.COLLISION);
    if (collisionTiles === null) {
      console.log(`encountered error while creating collision tiles from tiled`);
      return;
    }

    const collisionLayer = this.#map.createLayer(TILED_LAYER_NAMES.COLLISION, collisionTiles, 0, 0);
    if (collisionLayer === null) {
      console.log(`encountered error while creating collision layer using data from tiled`);
      return;
    }
    this.#collisionLayer = collisionLayer;
    this.#collisionLayer.setDepth(DEPTH.COLLIDER).setAlpha(DEBUG_COLLISION_ALPHA);

    const enemyCollisionLayer = this.#map.createLayer(TILED_LAYER_NAMES.ENEMY_COLLISION, collisionTiles, 0, 0);
    ...
    this.#enemyCollisionLayer.setDepth(DEPTH.COLLIDER).setVisible(false);
    ...
    // create game objects
    this.#createRooms(this.#map, TILED_LAYER_NAMES.ROOMS);

    // console.log(this.#objectsByRoomId);
    // console.log(this.#map);

    const rooms = getAllLayerNamesWithPrefix(this.#map, TILED_LAYER_NAMES.ROOMS).map((layerName: string) => {
      const roomIdStr = layerName.split('/')[1];
      return {
        name: layerName,
        roomId: parseInt(roomIdStr !== undefined ? roomIdStr : '0', 10),
      };
    });
    console.log(rooms);
    ...
    doorLayerNames.forEach((layer) => this.#createDoors(this.#map, layer.name, layer.roomId));
    switchLayerNames.forEach((layer) => this.#createButtons(this.#map, layer.name, layer.roomId));
    potLayerNames.forEach((layer) => this.#createPots(this.#map, layer.name, layer.roomId));
    chestLayerNames.forEach((layer) => this.#createChests(this.#map, layer.name, layer.roomId));
    enemyLayerNames.forEach((layer) => this.#createEnemies(this.#map, layer.name, layer.roomId));
  }
  ...
  #setupPlayer(): void {
    const startingDoor = assertNonNull(
    ...
    this.#player.setDepth(DEPTH.CHARACTER);
  }
  ...
  #createEnemies(map: Phaser.Tilemaps.Tilemap, layerName: string, roomId: number): void {
    ...
    for (const tiledObject of validTiledObjects) {
      ...
      if (tiledObject.type === 1) {
        ...
        spider.setDepth(DEPTH.CHARACTER);
  ...
  #handleRoomTransition(doorTrigger: Phaser.Types.Physics.Arcade.GameObjectWithBody): void {
    ...
    const doorTriggerNum = parseInt(doorTrigger.name);
    const door = this.#objectsByRoomId[this.#currentRoomId]?.doorMap[doorTriggerNum] as Door;
    ...
    if (isLevelName(modifiedLevelName)) {
      const sceneData: LevelData = {
        ...
        layerNameList: LEVEL_NAME_LIST[modifiedLevelName],
      };
      this.scene.start(SCENE_KEYS.GAME_SCENE, sceneData);
      return;
    }
    const targetDoor = assertNonNull(
      this.#objectsByRoomId[door.targetRoomId]?.doorMap[door.targetDoorId],
      'targetDoor'
    );
  ...
  #showObjectsInRoomById(roomId: number): void {
    if (this.#objectsByRoomId[roomId] === null || this.#objectsByRoomId[roomId] === undefined) {
      return;
    }
